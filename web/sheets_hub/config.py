...
_GOOGLE_ID_FROM_URL = re.compile(r"/spreadsheets/d/([a-zA-Z0-9-_]+)")
_DAV_PATH = re.compile(r"/remote\.php/dav/files/[^/]+/(.+)$", re.IGNORECASE)
_APPS_FILES_DIR = re.compile(r"[?&]dir=([^&]+)", re.IGNORECASE)
_APPS_FILES_FILE = re.compile(r"[?&]file=([^&]+)", re.IGNORECASE)


def parse_spreadsheet_id(url_or_id: str) -> str:
    """Нормализует путь к .xlsx на Nextcloud (или legacy Google ID)."""
    text = (url_or_id or "").strip()
    if not text:
        return ""
    # WebDAV: .../remote.php/dav/files/user/path/to/file.xlsx
    dav = _DAV_PATH.search(text.replace(" ", ""))
    if dav:
        from urllib.parse import unquote

        return unquote(dav.group(1)).lstrip("/")
    # UI Files: .../apps/files/?dir=/Folder&file=calendar.xlsx
    if "apps/files" in text or "index.php/apps/files" in text:
        from urllib.parse import unquote

        directory = ""
        filename = ""
        m_dir = _APPS_FILES_DIR.search(text)
        m_file = _APPS_FILES_FILE.search(text)
        if m_dir:
            directory = unquote(m_dir.group(1)).strip("/")
        if m_file:
            filename = unquote(m_file.group(1)).lstrip("/")
        ...
        if directory:
            return directory
    # Legacy Google (на случай старых конфигов) — оставляем ID.
    compact = re.sub(r"\s+", "", text)
    match = _GOOGLE_ID_FROM_URL.search(compact)
    if match:
        return match.group(1)
    # Обычный путь: SheetsHub/Calendar.xlsx
    return text.lstrip("/")
...
def credentials_email(path: Path) -> str:
    return account_label(path)


def install_credentials(source: Path, dest: Path | None = None) -> Path:
    from sheets_hub.auth import install_nextcloud_credentials

    return install_nextcloud_credentials(source, dest)
